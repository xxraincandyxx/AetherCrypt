...
.PHONY: style tidy check-style test clean-logs clean

# Format all Python files recursively
style:
	@echo "Formatting Python files..."
	@find src/. -name '*.py' -exec ruff format --config pyproject.toml {} +
	@find tests/. -name '*.py' -exec ruff format --config pyproject.toml {} +
...
clean:
	...
	@find . -type f -name 'workflow' -delete
